...
По умолчанию сидер добавляет небольшой demo-набор для Filament. Для нагрузочного наполнения можно сразу сгенерировать большой объём данных через переменные окружения:

```bash
CALL_CENTER_CLIENTS=1000 CALL_CENTER_OPERATORS=200 CALL_CENTER_CALLS=10000 php artisan migrate:fresh --seed
```

Поддерживаются переменные:

- `CALL_CENTER_CLIENTS` — сколько дополнительных клиентов сгенерировать bulk-вставкой
- `CALL_CENTER_OPERATORS` — сколько дополнительных операторов сгенерировать bulk-вставкой
- `CALL_CENTER_CALLS` — сколько дополнительных звонков и связанных `outbox_events` сгенерировать bulk-вставкой

Bulk-часть сидера использует chunked insert, поэтому сценарии на `1000`/`10000` записей создаются заметно быстрее, чем поштучный `Model::create()`.
